<x-app-layout>
    ...
    <section class="why-that-section">
        <div class="why-that-container container">
            <div class="why-that-content"><h2 class="why-that-title">{{ trans('when-buy.title-2') }}</h2>
                <p class="why-that-text text">{{ trans('when-buy.text-2') }}</p>
            </div>
            <div class="slider-why swiper">
                <div class="slider-why-body swiper-wrapper">
                    @foreach($storiesSlider->lang->content as $key => $slide)
                        <div class="swiper-slide">
                            <div class="slider-why-slide">
                                <div class="slider-why-slide-top">
                                    <img class="slider-why-slide-img" src="/storage/{{ $storiesSlider->images[$key] ?? '' }}" alt="{{ $slide['title'] }}" title="{{ $slide['title'] }}">
                                </div>
                                <div class="slider-why-slide-info">
                                    <h4 class="slider-why-slide-title">{{ $slide['title'] }}</h4>
                                    <p class="slider-why-slide-text">{{ $slide['subtitle'] }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            <div class="why-that-content why-that-content-last">
                <h2 class="why-that-title">{{ trans('when-buy.title-3') }}</h2>
                <p class="why-that-text text">{{ trans('when-buy.text-3') }}</p>
            </div>
        </div>
    </section>
    ...
    <section class="video-section" id="manifest">
        <div class="container video-container">
            <div class="video-wrap">
                <iframe width="420" height="315" src="{{ $homeVideo }}"></iframe>
            </div>
        </div>
    </section>
    ...
    <section class="refund-section" id="how-work">
        <div class="container">
            <div class="refund-title-wrap"><h2 class="refund-title">{{ trans('refund.title') }}</h2></div>
        </div>
            ...
            <div class="refund-body">
                @for($i = 1; $i <= 4; $i++)
                    <div class="refund-card">
                        <div class="refund-card-top">
                            <h4 class="refund-card-title">{{ trans('refund.card-' . $i . '-title') }}</h4>
                            <p class="refund-card-top-text">{{ trans('refund.card-' . $i . '-description') }}</p>
                        </div>
                        <div class="refund-card-bottom"><span class="refund-card-percent">{{ trans('refund.card-' . $i . '-percent') }}</span>
                            <p>{{ trans('refund.card-' . $i . '-description-2') }}</p>
                            <div class="refund-card-percent-wrap">
                                <span class="refund-card-percent">{{ trans('refund.card-' . $i . '-percent-2') }}</span>
                                <span class="refund-card-percent-text">{{ trans('refund.card-' . $i . '-percent-2-text') }}</span>
                            </div>
                            <p>{{ trans('refund.card-' . $i . '-description-3') }}</p>
                        </div>
                    </div>
                @endfor
                ...
                <div class="refund-card-wrap">
                    <div class="refund-card refund-card-blue">
                        <div class="refund-card-top">
                            <img class="refund-card-img" src="/images/consumption.svg" alt="{{ trans('refund.card-5-title') }}"></div>
                        <div><h4 class="refund-card-title">{{ trans('refund.card-5-title') }}</h4></div>
                    </div>
                    <div class="refund-card">
                        <div class="refund-card-top">
                            <img class="refund-card-img" src="/images/processing.svg" alt="{{ trans('refund.card-6-title') }}"></div>
                        <div>
                            <h4 class="refund-card-title">{{ trans('refund.card-6-title') }}</h4>
                            <p class="refund-card-top-text">{{ trans('refund.card-6-description') }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section class="manufacturer-section" id="presspack">
        <div class="container">
            <div class="manufacturer-title-wrap">
                <h2 class="manufacturer-title">{{ trans('manufacturer.title') }}</h2>
                <p class="manufacturer-subtitle text">{{ trans('manufacturer.description') }}</p>
            </div>
            <div class="manufacturer-body">
                <div class="action">
                    ...
                    <p class="action-info">{{ trans('manufacturer.card-1-description') }}</p>
                </div>
                <div class="action">
                    ...
                    <p class="action-info">{{ trans('manufacturer.card-2-description') }}</p>
                </div>
            </div>
            <p class="manufacturer-subtitle text">{{ trans('manufacturer.bottom-description') }}</p>
        </div>
    </section>
        ...
        <div class="container">
            <div class="partners-items">
                @foreach($partners as $partner)
                    <a href="{{ $partner->lang->url }}" target="_blank" class="partners-item">
                        <img class="partners-img" src="/storage/{{ $partner->image }}" alt="{{ $partner->lang->title }}" title="{{ $partner->lang->title }}">
                    </a>
                @endforeach
            </div>
        </div>
